...
public class CarAIHandler : MonoBehaviour
{
    ...
    void FollowWaypoints()
    {
        ...
        if (currentWaypoint != null)
        {
            //Set the target position of for the AI, based on the line between the waypoints

            if (previousWaypoint == null)
                targetPosition = currentWaypoint.transform.position;
            else
            {
                targetPosition = FindNearestPointOnLine(previousWaypoint.transform.position, currentWaypoint.transform.position, transform.position);
    ...
    bool IsCarsInFrontOfAICar(out Vector3 position, out Vector3 otherCarRightVector)
    {
        ...
        //Perform the circle cast in front of the car with a slight offset forward and use the same layer as the object is corrently on.
        RaycastHit2D raycastHit2d = Physics2D.CircleCast(transform.position + transform.up * 0.5f, 10.2f, transform.up, 12, 1 << polygonCollider2D.gameObject.layer);
    ...
    void AvoidCars(Vector2 vectorToTarget, out Vector2 newVectorToTarget)
    {
        if (IsCarsInFrontOfAICar(out Vector3 otherCarPosition, out Vector3 otherCarRightVector))
        {
            ...
            //Ensure that we limit the value to between 70% and 100% as we always want the AI to desire to reach the waypoint.  
